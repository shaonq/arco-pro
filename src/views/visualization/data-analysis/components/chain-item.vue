  ...
  const barChartOptionsFactory = () => {
    ...
    const chartOption = useChartOption(() => {
      return {
        ...
        series: { name: 'total', data, type: 'bar', barWidth: 7, itemStyle: { borderRadius: 2 } },
      };
    });
  ...
  const lineChartOptionsFactory = () => {
    ...
    const chartOption = useChartOption(() => {
      return {
        ...
        tooltip: { show: true, trigger: 'axis' },
        series: [
          { name: '2001', data: data.value[0], type: 'line', showSymbol: false, smooth: true, lineStyle: { color: '#165DFF', width: 3 } },
          { name: '2002', data: data.value[1], type: 'line', showSymbol: false, smooth: true, lineStyle: { color: '#6AA1FF', width: 3, type: 'dashed' } },
        ],
      };
    });
    return { data, chartOption };
  ...
  const pieChartOptionsFactory = () => {
    ...
    const chartOption = useChartOption(() => {
      return {
        ...
        legend: { show: true, top: 'center', right: '0', orient: 'vertical', icon: 'circle', itemWidth: 6, itemHeight: 6, textStyle: { color: '#4E5969' } },
        tooltip: { show: true },
        series: [{ name: '总计', type: 'pie', radius: ['50%', '70%'], label: { show: false }, data }],
      };
    });
    return { data, chartOption };
  };
